...
Prepare environment: Ensure your SiteGround plan supports PHP 8.2+, MySQL 8 and has pdo_mysql enabled. Create a new MySQL database and user via SiteGround’s control panel.

Upload files: Copy the contents of sacci_brand_hub/ to your site’s document root (e.g., public_html/brandhub). For security, place the storage/ directory outside of the public directory if SiteGround allows custom directory structures.

Set permissions: Ensure the web server can write to the storage/ directory and the root directory to create the .env file during installation. You can typically set directories to 755 and files to 644.

Run the installer: Navigate to https://yourdomain.com/brandhub/install/. Enter your database credentials and the email/password for the first Super Admin user. The installer writes the .env file, runs the migrations and seeds initial data. After success, delete the install/ folder as instructed.

Install dependencies: From a terminal (SSH or local), run composer install in the project root to fetch PHPMailer. If you cannot run Composer on SiteGround, you may vendor PHPMailer locally and upload the vendor/ folder.

Configure mail: Edit .env to add your SMTP host, port and credentials. You can test by creating a ticket and ensuring notification emails send.

Set up cron (optional): Configure a cron job to run maintenance scripts, such as pruning old sessions or sending SLA reminders.

Go live: Point DNS or configure a subdomain to the project directory. Use HTTPS and ensure the .htaccess file is active to route all requests through index.php.
